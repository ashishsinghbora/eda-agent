...
async def check_p_ready_drops_low_when(dut):
    """Cocotb checker: ready drops low when valid is asserted and fifo is full"""
    while True:
        await RisingEdge(dut.clk)
        if int(dut.rst_n.value) == 1:
            if int(dut.valid.value) and int(dut.fifo_full.value):
                assert int(dut.ready.value) == 0, (
                    f"Assertion p_ready_drops_low_when failed: dut.ready is {dut.ready.value}, "
                    f"expected 0 when valid && fifo_full is active"
                )
