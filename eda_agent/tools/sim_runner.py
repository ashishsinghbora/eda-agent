"""Headless Icarus Verilog and Cocotb simulation harness and diagnostic analyzer."""
...
from eda_agent.runners.simulation_runner import (
    FailureType,
...
class SimRunner(BaseTool):
    """Executes Icarus Verilog and Cocotb test suites in headless sandboxes."""

    @classmethod
    def is_available(cls) -> bool:
        """Return True if Icarus Verilog (iverilog and vvp) is available."""
        return cls.find_binary("iverilog") is not None and cls.find_binary("vvp") is not None
    ...
    def run(
        ...
        # Check make binary
        make_bin = cls.find_binary("make")
        if not make_bin:
            return SimulationResult(
                success=False,
                exit_code=127,
                duration_seconds=0.0,
                stderr="GNU Make ('make') was not found on system PATH. Install build-essential / make.",
                diagnostics=SimulationDiagnostics(
                    failure_type=FailureType.COMPILATION_ERROR,
                    error_summary="GNU Make ('make') was not found on PATH.",
                    raw_stderr="GNU Make ('make') was not found on PATH."
                )
            )

        if clean:
            cls.execute_command(["make", "clean"], cwd=work_path, timeout=30, extra_env=env)

        exec_res: ToolResult = cls.execute_command(
            cmd=["make"],
            cwd=work_path,
            timeout=timeout,
            extra_env=env,
    ...
    def diagnose_failure(
        ...
        if exit_code == -1 or "Simulation timed out" in stderr or "timed out" in combined_logs.lower():
            return SimulationDiagnostics(
                ...
                error_summary="Simulation exceeded timeout threshold (potential deadlock or infinite loop).",
                raw_stderr=stderr,
                raw_stdout=stdout
            )
        ...
        if "error: " in combined_logs or "iverilog:" in combined_logs or "syntax error" in combined_logs.lower():
            ...
            summary = "\n".join(comp_errors[:5]) if comp_errors else "RTL or Simulation compilation failed."
            return SimulationDiagnostics(
                failure_type=FailureType.COMPILATION_ERROR,
        ...
        if report and (report.failures > 0 or report.errors > 0):
            for tc in report.test_cases:
                if not tc.passed:
                    fail_type = FailureType.RUNTIME_ERROR if tc.failure_type == "Error" else FailureType.ASSERTION_ERROR
                    return SimulationDiagnostics(
